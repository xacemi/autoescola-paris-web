...
export default async function Home({
  ...
  const [{ data: noticies }, { data: permisos }, { data: horaris }, videos] = await Promise.all([
    supabase.from('noticies').select('id, titol, contingut, data_publicacio').eq('publicada', true).order('data_publicacio', { ascending: false }).limit(3),
    supabase.from('preus_permisos').select('id, tipus_permis, descripcio, preu_total_aproximat').eq('actiu', true).order('tipus_permis'),
    supabase.from('horaris').select('id, dia_setmana, hora_inici, hora_fi, tipus').eq('actiu', true).order('dia_setmana'),
    fetchYouTubeVideos(),
  ])
  ...
  return (
    <main className="flex flex-col min-h-screen bg-[#F4F6FB] text-[#1F2937]">
      ...
      {/* ── HERO ── */}
      <section id="inici" className="relative text-white pt-12 pb-10 px-4 text-center" style={{ backgroundImage: "url('/images/Oficinas.png')", backgroundSize: 'cover', backgroundPosition: 'center' }}>
        ...
        <div className="relative z-10 flex flex-col items-center gap-4 max-w-sm mx-auto">
          ...
          <div>
            ...
            <p className="text-[#F59E0B] font-semibold text-sm tracking-widest uppercase mt-1">Lleida · Mollerussa · Desde 1961</p>
          </div>
        </div>
      </section>
      ...
      <section className="px-4 py-6 -mt-1 bg-[#0110D6]">
        <div className="max-w-md mx-auto grid grid-cols-3 gap-3 pb-2">
          {[
            ...
            <a key={item.href} href={item.href} className="bg-white rounded-2xl py-4 px-2 flex flex-col items-center gap-2 shadow-md hover:shadow-lg border-2 border-transparent hover:border-[#F59E0B] active:scale-95 transition-all">
      ...
      <section id="horarios" className="relative py-16 px-4" style={{ backgroundImage: "url('/images/ConsultaTeorico.png')", backgroundSize: 'cover', backgroundPosition: 'center' }}>
        ...
        <div className="relative z-10 max-w-3xl mx-auto">
          <h2 className="text-2xl sm:text-3xl font-bold text-white mb-2 text-center">Horarios de clase On Line</h2>
          <p className="text-white/80 text-center mb-10 text-sm">Clases online adaptadas a tu agenda</p>
        ...
        <div className="relative z-10 max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              ...
              <a key={p.code} href={p.href} target="_blank" rel="noreferrer" className="bg-white rounded-2xl border-2 border-zinc-100 hover:border-[#0110D6] shadow-sm hover:shadow-md transition-all group flex flex-col overflow-hidden">
                ...
                <div className="p-3 flex flex-col items-center text-center flex-1">
                  ...
                  <span className="bg-[#F59E0B] text-white text-xs font-bold px-3 py-1.5 rounded-full w-full text-center group-hover:bg-[#D97706] transition-colors">Más información</span>
                </div>
              </a>
            ))}
          </div>
      ...
      <section id="alumnos" className="relative py-16 px-4" style={{ backgroundImage: "url('/images/Alumnos.png')", backgroundSize: 'cover', backgroundPosition: 'center' }}>
        ...
        <div className="relative z-10 max-w-lg mx-auto text-center">
          ...
          <a href="/alumnes" className="inline-flex items-center gap-2 bg-white text-[#F59E0B] font-bold px-8 py-4 rounded-2xl shadow-lg hover:shadow-xl active:scale-95 transition-all text-base">
          ...
          <div className="flex flex-col gap-4">
            <details className="bg-white rounded-2xl shadow-md overflow-hidden group">
              <summary className="flex items-center gap-3 px-5 py-4 cursor-pointer select-none">
                ...
                <svg className="w-5 h-5 text-[#0110D6] transition-transform group-open:rotate-180" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
              </summary>
              <div className="flex flex-col gap-2 px-4 pb-4">
                {[
                  ...
                  <a key={loc.label} href={`https://www.google.com/maps?q=${loc.lat},${loc.lng}`} target="_blank" rel="noreferrer" className="flex items-center gap-3 bg-blue-50 hover:bg-blue-100 rounded-xl px-4 py-3 transition-colors">
                    <span className="text-lg">📍</span>
                    <div className="flex-1">
                      <p className="font-semibold text-[#1F2937] text-sm">{loc.label}</p>
                      <p className="text-xs text-zinc-500">{loc.addr}</p>
                    </div>
                    <svg className="w-4 h-4 text-[#0110D6]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
                  </a>
                ))}
              </div>
            </details>
      ...
      <section id="videos" className="relative py-16 px-4" style={{ backgroundImage: "url('/images/Matricula%20On%20line.png')", backgroundSize: 'cover', backgroundPosition: 'center' }}>
        ...
        <div className="relative z-10 max-w-5xl mx-auto">
              ...
              {videos.map((v) => (
                <div key={v.id} className="bg-white rounded-2xl border border-zinc-100 overflow-hidden shadow-sm hover:shadow-md transition-shadow group">
                  ...
                  <div className="p-4 flex flex-col gap-3">
                    ...
                    {v.published && <p className="text-xs text-zinc-400">{new Date(v.published).toLocaleDateString('es-ES', { day: 'numeric', month: 'long', year: 'numeric' })}</p>}
                    <a href={`https://www.youtube.com/watch?v=${v.id}`} target="_blank" rel="noreferrer" className="bg-[#F59E0B] hover:bg-[#D97706] text-white text-xs font-bold px-4 py-2 rounded-full transition-colors text-center">Ver en YouTube</a>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="text-center mt-8">
            <a href="https://www.youtube.com/@ParisAutoescola" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 border-2 border-white text-white hover:bg-white hover:text-[#0110D6] font-semibold px-6 py-2.5 rounded-full transition-colors text-sm">
            ...
            <div>
              {enviat ? (
                ...
                <form action={enviarContacte} className="flex flex-col gap-4 bg-white rounded-2xl p-6 shadow-sm border border-purple-100">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <select name="permis_interes" className="w-full border border-zinc-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#0110D6] focus:border-transparent text-[#1F2937]">
                        ...
                        {permisos?.map((p) => (<option key={p.id} value={p.tipus_permis}>{p.tipus_permis}</option>))}
                      </select>
                    </div>
                  </div>
                  <div>
                  ...
                  <button type="submit" className="bg-[#F59E0B] hover:bg-[#D97706] text-white font-bold px-6 py-3 rounded-xl transition-colors text-sm">Enviar mensaje</button>
                </form>
              )}
            </div>
            <div className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 mb-8">
            <div>
              ...
              <div className="flex gap-2">
                <a href="https://www.facebook.com/ParisAutoescola/" target="_blank" rel="noreferrer" aria-label="Facebook" className="w-8 h-8 bg-white/10 hover:bg-[#F59E0B] rounded-lg flex items-center justify-center transition-colors">
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" /></svg>
                </a>
                <a href="https://www.instagram.com/autoescolaparis" target="_blank" rel="noreferrer" aria-label="Instagram" className="w-8 h-8 bg-white/10 hover:bg-[#F59E0B] rounded-lg flex items-center justify-center transition-colors">
                ...
                <a href="https://www.tiktok.com/@autoescolaparis" target="_blank" rel="noreferrer" aria-label="TikTok" className="w-8 h-8 bg-white/10 hover:bg-[#F59E0B] rounded-lg flex items-center justify-center transition-colors">
